...
public class WordXml {
    ...
    private static String formContent(ContentPojo contentPojo,Integer startIndex,Integer endIndex){
        ...
        for(int i=startIndex;i<=endIndex;i++){
            ...
            if(p.getElement_type().equals("text")||p.getElement_type().equals("title")){
                sb.append("<p>");
                System.out.println(p.getText());
                sb.append(p.getText().replaceAll("\n",""));
                sb.append("</p>");
                sb.append("\n");
            }else if(p.getElement_type().equals("table")){
                //形成表格
            ...
            }else if(p.getElement_type().equals("pic")){
                //形成图片
...
//            }
        ...
        if(extractPojo==null){
            extractPojo = SettingReader.getDocExtract();
        }
...
//                        list.add(new Tu.Tuple2<>(finalTitle,"无内容"));
    ...
    /**
     * 判断当前是否是隐藏层，虽然word暂时没有添加隐含层，也暂时放一个该方法
     * @param level
     * @return 是或者不是
     */
    ...
    public static Document buildXml(ContentPojo contentPojo) {
        ...
        for(ContentPojo.Text t:outList){
        ...
        while (pos != outList.size()) {
            //pos找到第一个标题
            ContentPojo.Text p = outList.get(pos);
        ...
        if(curE!=null){
            ...
            int tailIndex = 0;
            Integer titleIndex = Integer.parseInt(curE.attribute("key").getValue());
            for(int i=outList.size()-1;i>titleIndex;i--){
                ContentPojo.Text cur = outList.get(i);
                ContentPojo.Text pre = outList.get(i - 1);

                List<ContentPojo.WordStyleStruct> wordStyleStructs = pre.getWordStyleStructs();
                if(!CollectionUtils.isEmpty(wordStyleStructs)){
                    String spacing = wordStyleStructs.get(0).getSpacing();
                    if(!StringUtils.isEmpty(spacing)){
                        Optional<String> afterSpacingOption = Arrays.stream(spacing.split(",")).map(x -> {
                            String[] split = x.split(":");
                            if (split.length == 2) {
                                return new Tu.Tuple2<>(split[0], split[1]);
                            } else {
                                return new Tu.Tuple2<>("", "");
                            }
                        }).filter(x -> {
                            return x.getKey().equals("A");
                        }).map(Tu.Tuple2::getValue).findFirst();
                        if(afterSpacingOption.isPresent()){
                            String afterSpacingStr = afterSpacingOption.get();
                            if(afterSpacingStr.matches("[0-9]+")){
                                int afterSpacing = Integer.parseInt(afterSpacingStr);
                                //距离达到2000就认为和上一个元素距离过大
                                if(afterSpacing>2000){
                                    tailIndex = i;
                                    break;
                                }
                            }
